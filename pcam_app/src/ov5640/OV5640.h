...
config_word_t const cfg_advanced_awb_[] = {
    // Enable Advanced AWB
    {0x3406, 0x00}, {0x5192, 0x04}, {0x5191, 0xf8}, {0x518d, 0x26},
    {0x518f, 0x42}, {0x518e, 0x2b}, {0x5190, 0x42}, {0x518b, 0xd0},
    {0x518c, 0xbd}, {0x5187, 0x18}, {0x5188, 0x18}, {0x5189, 0x56},
    {0x518a, 0x5c}, {0x5186, 0x1c}, {0x5181, 0x50}, {0x5184, 0x20},
    {0x5182, 0x11}, {0x5183, 0x00}, {0x5001, 0x03}};
...
config_word_t const cfg_disable_awb_[] = {{0x5001, 0x02}};

config_word_t const cfg_720p_60fps_[] =
    {  // 1280 x 720 binned, RAW10, MIPISCLK=280M, SCLK=56Mz, PCLK=56M
       // PLL1 configuration
       //[7:4]=0010 System clock divider /2, [3:0]=0001 Scale divider for MIPI
       ///1
        {0x3035, 0x21},
        ...
        {0x3037, 0x05},
        //[5:4]=01 PCLK root divider /2, [3:2]=00 SCLK2x root divider /1,
        ...
        //[7:4]=0x3 horizontal odd subsample increment, [3:0]=0x1 horizontal
        //even subsample increment
        {0x3814, 0x31},
        //[7:4]=0x3 vertical odd subsample increment, [3:0]=0x1 vertical even
        //subsample increment
        {0x3815, 0x31},
        ...
        // little MIPI shit: global timing unit, period of PCLK in ns *
        // 2(depends on # of lanes)
        {0x4837, 36},  // 1/56M*2
...
config_word_t const cfg_1080p_15fps_[] =
    {  // 1920 x 1080 @ 15 fps, RAW10, MIPISCLK=210, SCLK=42MHz, PCLK=42M
       //  PLL1 configuration
       //  [7:4]=0100 System clock divider /4, [3:0]=0001 Scale divider for MIPI
       //  /1
        {0x3035, 0x41},
        ...
        // [7:4]=0x1 horizontal odd subsample increment, [3:0]=0x1 horizontal
        // even subsample increment
        {0x3814, 0x11},
...
config_word_t const cfg_1080p_30fps_336M_mipi_[] =
    {  // 1920 x 1080 @ 30fps, RAW10, MIPISCLK=672, SCLK=67.2MHz, PCLK=134.4M
       // PLL1 configuration
       //[7:4]=0001 System clock divider /1, [3:0]=0001 Scale divider for MIPI
       ///1
...
config_word_t const cfg_1080p_30fps_336M_1lane_mipi_[] =
    {  // 1920 x 1080 @ 30fps, RAW10, MIPISCLK=672, SCLK=67.2MHz, PCLK=134.4M
       ...
       ///1
        ...
        //[7:5]=001 One lane mode, [4]=0 MIPI HS TX no power down, [3]=0 MIPI LP
        //RX no power down, [2]=1 MIPI enable, [1:0]=10 Debug mode; Default=0x58
        {0x300e, 0x25},
...
config_word_t const cfg_init_[] = {
    ...
    //              +----------------+        +------------------+
    //              +---------------------+        +---------------------+
    // XVCLK         | PRE_DIV0       |        | Mult (4+252)     |         |
    // Sys divider (0=16)  |        | MIPI divider (0=16) |
    //+-------+-----> 3037[3:0]=0001 +--------> 3036[7:0]=0x38   +--------->
    //3035[7:4]=0001      +--------> 3035[3:0]=0001      | 12MHz   |     | / 1
    // | 12MHz  | * 56             | 672MHz  | / 1                 | 672MHz | /
    // 1                 |
    //        |     +----------------+        +------------------+
    //        +----------+----------+        +----------+----------+ | | | | |
    //        MIPISCLK|672MHz | |                              | |
    //        +----------------+        +------------------+
    //        +----------v----------+        +----------v----------+ |     |
    //        PRE_DIVSP      |        | R_DIV_SP         |         | PLL R
    //        divider       |        | MIPI PHY            | MIPI_CLK
    //        +-----> 303d[5:4]=01   +--------> 303d[2]=0 (+1)   |         |
    //        3037[4]=1 (+1)      |        |                     +------->
    //              | / 1.5          |  8MHz  | / 1              |         | / 2
    //              |        | / 2                 | 336MHz
    //              +----------------+        +---------+--------+
    ...
    //              +----------------+        +---------v--------+
    //              +----------v----------+        +---------------------+ | SP
    //              divider     |        | Mult             |         | BIT div
    //              (MIPI 8/10) |        | SCLK divider        | SCLK |
    //              303c[3:0]=0x1  +<-------+ 303b[4:0]=0x19   |         |
    ...
    //                       | |               | | |               | | | |
    //              +--------v-------+ +----------v----------+    |
    //              +---------------------+ | R_SELD5 div    | ADCCLK | PCLK div
    //              |    |   | SCLK2x divider      | | 303d[1:0]=001  +------->
    //              | 3108[5:4]=00 (2^)   |    +---> 3108[3:2]=00 (2^) +------->
    //              | / 1            | 200MHz                              | / 1
    //              |        | / 1                 | 168MHz
    //              +----------------+ +----------+----------+
    //              +---------------------+
    ...
    {0x3037, 0x11},
    //[5:4]=00 PCLK root divider /1, [3:2]=00 SCLK2x root divider /1, [1:0]=01
    //SCLK root divider /2
    {0x3108, 0x01},
    // PLL2 configuration
    ...
    //[7:5]=010 Two lane mode, [4]=0 MIPI HS TX no power down, [3]=0 MIPI LP RX
    ...
    //[7]=1 LENC correction enabled, [5]=1 RAW gamma enabled, [2]=1 Black pixel
    //cancellation enabled, [1]=1 White pixel cancellation enabled, [0]=1 Color
...
config_modes_t const modes[] = {
    ...
    {MAP_ENUM_TO_CFG(MODE_1080P_1920_1080_30fps_336M_MIPI,
                     cfg_1080p_30fps_336M_mipi_)},
    {MAP_ENUM_TO_CFG(MODE_1080P_1920_1080_30fps_336M_1LANE_MIPI,
                     cfg_1080p_30fps_336M_1lane_mipi_)},
};
...
class OV5640 {
  ...
  std::stringstream ss;
  ss << "Got " << std::hex << id_h << id_l << ". Expected " << dev_ID_h_ <<
  dev_ID_l_;
             */
            char msg[100];
        ...
        size_t i;
        for (i = 0; i < sizeof(OV5640_cfg::cfg_init_) /
                            sizeof(OV5640_cfg::cfg_init_[0]);
             ++i) {
            writeReg(OV5640_cfg::cfg_init_[i].addr,
    ...
    Errc set_mode(OV5640_cfg::mode_t mode) {
        if (mode >= OV5640_cfg::mode_t::MODE_END) return ERR_LOGICAL;
    ...
    Errc set_awb(OV5640_cfg::awb_t awb) {
        if (awb >= OV5640_cfg::awb_t::AWB_END) return ERR_LOGICAL;
        //[7]=0 Software reset; [6]=1 Software power down; Default=0x02
        writeReg(0x3008, 0x42);
    ...
    Errc set_isp_format(OV5640_cfg::isp_format_t isp) {
        if (isp >= OV5640_cfg::isp_format_t::ISP_END) return ERR_LOGICAL;
        //[7]=0 Software reset; [6]=1 Software power down; Default=0x02
        writeReg(0x3008, 0x42);
    ...
    void readReg(uint16_t reg_addr, uint8_t& buf) {
        for (auto retry_count = retry_count_; retry_count > 0; --retry_count) {
            try {
                auto buf_addr = std::vector<uint8_t>{(uint8_t)(reg_addr >> 8),
                                                     (uint8_t)reg_addr};
    ...
    void writeReg(uint16_t reg_addr, uint8_t const reg_data) {
        for (auto retry_count = retry_count_; retry_count > 0; --retry_count) {
            try {
                auto buf = std::vector<uint8_t>{(uint8_t)(reg_addr >> 8),
    ...
    class HardwareError : public std::runtime_error {
       public:
        ...
        HardwareError(Errc errc, char const* msg)
   ...
   private:
    void usleep(uint32_t time) {  // TODO couldn't think of anything better
