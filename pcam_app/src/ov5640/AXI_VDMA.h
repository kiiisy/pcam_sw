...
/*!
 * \brief Driver class for Xilinx AXI VDMA IP. Needs to have stable clocks
 * before instantiation to be able to complete hardware reset.
 */
...
class AXI_VDMA {
    typedef struct vdma_context_t {
        ...
        /* The XAxiVdma_DmaSetup structure contains all the necessary
         * information to start a frame write or read. */
        ...
        /* The counter to tell VDMA on how many frames the interrupt should
         * happen*/
   ...
   public:
    ...
    AXI_VDMA(uint16_t dev_id, uint32_t frame_buf_base_addr, IrptCtl& irpt_ctl,
        ...
        : rd_handler_(
              std::bind(&AXI_VDMA::readHandler, this, std::placeholders::_1)),
          wr_handler_(
              std::bind(&AXI_VDMA::writeHandler, this, std::placeholders::_1)),
          rd_err_handler_(std::bind(&AXI_VDMA::readErrorHandler, this,
                                    std::placeholders::_1)),
          wr_err_handler_(std::bind(&AXI_VDMA::writeErrorHandler, this,
                                    std::placeholders::_1)),
          context_{},
        ...
        Status =
            XAxiVdma_CfgInitialize(&drv_inst_, psConf, psConf->BaseAddress);
        if (Status != XST_SUCCESS) {
            throw std::runtime_error(__FILE__ ":" LINE_STRING);
        }

        // Set error interrupt error handlers, which for some reason need
        // completion handler defined too
        XAxiVdma_SetCallBack(
            &drv_inst_, XAXIVDMA_HANDLER_GENERAL,
        ...
        XAxiVdma_SetCallBack(
            &drv_inst_, XAXIVDMA_HANDLER_ERROR,
    ...
    void resetRead() {
        ...
        //		while
        //(XAxiVdma_ChannelIsRunning(&drv_inst_.ReadChannel)) ;
    ...
    void resetWrite() {
        ...
        //		while
        //(XAxiVdma_ChannelIsRunning(&drv_inst_.WriteChannel)) ;
    ...
    void configureRead(uint16_t h_res, uint16_t v_res) {
        XStatus status;
        context_.ReadCfg.HoriSizeInput =
            h_res * drv_inst_.ReadChannel.StreamWidth;
        ...
        status =
            XAxiVdma_DmaConfig(&drv_inst_, XAXIVDMA_READ, &context_.ReadCfg);
        ...
        for (int iFrm = 0; iFrm < drv_inst_.MaxNumFrames; ++iFrm) {
            ...
            // memset((void*)addr,0,context_.ReadCfg.HoriSizeInput *
            // context_.ReadCfg.VertSizeInput);
            addr +=
                context_.ReadCfg.HoriSizeInput * context_.ReadCfg.VertSizeInput;
        }
        status = XAxiVdma_DmaSetBufferAddr(
            &drv_inst_, XAXIVDMA_READ, context_.ReadCfg.FrameStoreStartAddr);
        if (XST_SUCCESS != status) {
            throw std::runtime_error(__FILE__ ":" LINE_STRING);
        }
    ...
    void configureWrite(uint16_t h_res, uint16_t v_res) {
        ...
        context_.WriteCfg.HoriSizeInput =
            h_res * drv_inst_.WriteChannel.StreamWidth;
        ...
        context_.WriteCfg.FixedFrameStoreAddr =
            0;  // ignored, since we circle through buffers
        status =
            XAxiVdma_DmaConfig(&drv_inst_, XAXIVDMA_WRITE, &context_.WriteCfg);
        if (XST_SUCCESS != status) {
            throw std::runtime_error(__FILE__ ":" LINE_STRING);
        }
        uint32_t addr = frame_buf_base_addr_;
        ...
        status = XAxiVdma_DmaSetBufferAddr(
            &drv_inst_, XAXIVDMA_WRITE, context_.WriteCfg.FrameStoreStartAddr);
        if (XST_SUCCESS != status) {
            throw std::runtime_error(__FILE__ ":" LINE_STRING);
        }
        // Clear errors in SR
